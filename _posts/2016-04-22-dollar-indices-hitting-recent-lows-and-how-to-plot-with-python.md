...
Discrepancies between even the broad index and the way the dollar is actually traded are inevitable. For example, the broad index weight in 2016 for Canada is 12.664 percent (broad index weights are updated yearly), whereas [year-to-date](https://www.census.gov/foreign-trade/statistics/highlights/toppartners.html), 15.2 percent of total U.S. trade has been with Canada. From a share-of-total-U.S.-trade perspective, the broad index is somewhat overweight the Yuan, Euro, and Yen, and underweight the Canadian dollar and Mexican peso.
...
Lastly, we can use matplotlib to plot the data.
    ...
    fig.text(0.65, 0.16, 'bd-econ.com')
